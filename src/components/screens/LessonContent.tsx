...
import svgPaths from "../../imports/svg-mrjxnxchwf";
...
function Group11869({ onCreateQuiz }: { onCreateQuiz: () => void }) {
  return (
...
function Frame13({ 
  ...
  return (
    <div className="content-stretch flex gap-[20px] items-center relative shrink-0">
      <button onClick={onH1Click} className="cursor-pointer hover:opacity-70 transition-opacity">
        <BiTypeH1 />
      </button>
      <button onClick={onBoldClick} className="cursor-pointer hover:opacity-70 transition-opacity">
        <FiBold />
      </button>
      <button onClick={onItalicClick} className="cursor-pointer hover:opacity-70 transition-opacity">
        <FiItalic />
      </button>
      <button onClick={onEditClick} className="cursor-pointer hover:opacity-70 transition-opacity">
        <FiEdit3 />
      </button>
      ...
      <button onClick={onLinkClick} className="cursor-pointer hover:opacity-70 transition-opacity">
        <FiLink2 />
      </button>
    </div>
  );
...
function Frame14({ 
  ...
  const topPosition = hasImage ? '404px' : '500px';
  
  return (
    <div className={`absolute bg-white box-border content-stretch flex flex-col gap-[10px] items-start left-[484px] px-[9px] py-[8px] rounded-[5.185px] shadow-[0px_0px_5.185px_1.037px_rgba(0,0,0,0.23)]`} style={{ top: topPosition }}>
      <Frame13 
        onH1Click={onH1Click}
...
function ImageWithCaption({ imageUrl, caption }: { imageUrl: string; caption: string }) {
  return (
    <div className="absolute contents left-[33px] top-[144px]" data-name="Image with Caption">
      <div className="absolute h-[369.549px] left-[33px] rounded-[8px] top-[144px] w-[584.648px]" data-name="uploaded-image">
        <div aria-hidden="true" className="absolute inset-0 pointer-events-none rounded-[8px]">
          <div className="absolute bg-[#c4c4c4] inset-0 rounded-[8px]" />
          <img alt="Uploaded content" className="absolute max-w-none object-50%-50% object-cover rounded-[8px] size-full" src={imageUrl} />
        </div>
      </div>
      {caption && (
        <p className="absolute font-['Poppins:Medium',_sans-serif] h-[19px] leading-[26px] left-[33px] not-italic text-[#9b9b9b] text-[14px] top-[518px] w-[261px]">{caption}</p>
      )}
    </div>
  );
}

function TextField({ value, onChange, imageUrl, imageCaption }: { 
  value: string; 
  ...
  const height = hasImage ? '560px' : '377px';
  const topPosition = hasImage ? '456px' : '552px';
  
  return (
    <div className="absolute bg-white left-[86px] rounded-[6.594px] w-[721px]" style={{ height, top: topPosition }} data-name="Text field">
      <div className="overflow-clip relative rounded-[inherit] w-[721px]" style={{ height }}>
        <textarea
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="absolute font-['Poppins:Medium',_sans-serif] leading-[30px] left-[33px] not-italic text-[#232323] text-[20px] top-[37px] w-[620px] bg-transparent border-none outline-none resize-none"
          style={{ height: hasImage ? '80px' : 'calc(100% - 74px)' }}
          placeholder="Type your lesson content here..."
        />
        {hasImage && imageUrl && (
          <ImageWithCaption imageUrl={imageUrl} caption={imageCaption || ''} />
        )}
      </div>
...
function TextArea({ value, onChange, imageUrl, imageCaption }: { 
  value: string; 
  onChange: (value: string) => void;
  imageUrl?: string | null;
  imageCaption?: string;
  ...
  return (
    <div className="absolute contents left-[86px]" data-name="Text Area">
      <TextField value={value} onChange={onChange} imageUrl={imageUrl} imageCaption={imageCaption} />
    </div>
  );
}
...
function Group11868({ 
  ...
  const hasImage = !!imageUrl;
  const topPosition = hasImage ? '404px' : '500px';
  const labelTopPosition = hasImage ? '404px' : '500px';
  
  return (
    <div className={`absolute contents left-[84px]`} style={{ top: topPosition }}>
      <div className={`absolute flex flex-col font-['Poppins:SemiBold',_sans-serif] justify-center leading-[0] left-[84px] not-italic text-[#232323] text-[24px] text-nowrap`} style={{ top: labelTopPosition }}>
        <p className="leading-[45.434px] whitespace-pre">Lesson Content</p>
      </div>
      <TextArea value={lessonContent} onChange={onContentChange} imageUrl={imageUrl} imageCaption={imageCaption} />
      <Frame14 
        onH1Click={onH1Click}
        onBoldClick={onBoldClick}
        onItalicClick={onItalicClick}
...
function TextField1({ value, onChange, maxLength }: { 
  ...
  return (
    <div className="absolute bg-white h-[52.749px] left-[86px] rounded-[6.594px] top-[393px] w-[583px]" data-name="Text field">
...
function TextArea1({ value, onChange, maxLength }: { 
  value: string; 
  onChange: (value: string) => void;
  maxLength: number;
}) {
  return (
    <div className="absolute contents left-[86px] top-[393px]" data-name="Text Area">
      <TextField1 value={value} onChange={onChange} maxLength={maxLength} />
    </div>
  );
}
...
function Group11860({ value, onChange, charCount }: { 
  ...
  return (
    <div className="absolute contents left-[84px] top-[341px]">
      <div className="absolute top-[341px] flex flex-col font-['Poppins:Medium',_sans-serif] justify-center leading-[0] left-[84px] not-italic text-[#9b9b9b] text-[13.535px] text-nowrap">
        <p className="leading-[25.622px] whitespace-pre">Max Char {charCount}/40</p>
      </div>
      <div className="absolute top-[293px] flex flex-col font-['Poppins:SemiBold',_sans-serif] justify-center leading-[0] left-[84px] not-italic text-[#232323] text-[24px] text-nowrap">
        <p className="leading-[45.434px] whitespace-pre">Lesson Name</p>
      </div>
      <TextArea1 value={value} onChange={onChange} maxLength={40} />
    </div>
  );
}

export function LessonContent({ onNavigate, data }: LessonContentProps) {
  const [lessonName, setLessonName] = useState(data?.lessonTitle || 'What is Space?');
  const [lessonContent, setLessonContent] = useState('Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ac nisi arcu ultrices sem viverra tincidunt nisl. Dapibus gravida est, et ultricies eget consequat. ');
  ...
  return (
    <div className="bg-white relative w-[1440px] min-h-screen" data-name="Lesson Content">
      <NavBarTeachers onAddInstitute={handleAddInstitute} />
      <BackNav onClick={handleBackToLessonDetails} />
      <Frame20 />
      <Group11869 onCreateQuiz={handleCreateQuiz} />
      <Group11868 
      ...
      <div className="absolute flex flex-col font-['Poppins:SemiBold',_sans-serif] justify-center leading-[0] left-[84px] not-italic text-[32px] text-black text-nowrap top-[267px] translate-y-[-50%]">
        <p className="leading-[normal] whitespace-pre">{lessonName}</p>
      </div>
